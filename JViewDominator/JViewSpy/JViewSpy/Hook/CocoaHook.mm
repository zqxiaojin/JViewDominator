...
//  iUCWEB
//
//  Created by Samuel yang on 10/20/11.
                                        ...
                                        const char* methodStr)
{
    ...
    if(methodStr)
    {
        ...
        result = class_addMethod(orgClass, newMethod, newMethodIMP, methodStr);
		if (result)
		{
...
NSString* getMethodString(const char* className, const char* selectorName)
{
	
	NSMethodSignature* method = [objc_getClass(className) instanceMethodSignatureForSelector:sel_registerName(selectorName)];
	NSMutableString* methodString = [[NSMutableString alloc] initWithUTF8String:[method methodReturnType]];
	for (int i = 0 , ic = [method numberOfArguments]; i < ic; ++i)
	{
		[methodString appendString:[NSString stringWithUTF8String:[method getArgumentTypeAtIndex:i]]];
	}
	return methodString;
}

bool OBJC_EXCHANGE_NEWCLASS_METHOD_AUTO(const char* orgClassStr,const char* orgSelector,
										const char* newClassStr,const char* newSelector)
{
	NSString* methodString = getMethodString(orgClassStr,orgSelector);
	bool result =  OBJC_EXCHANGE_NEWCLASS_METHOD_TEMPLATE<class_getInstanceMethod>(orgClassStr,orgSelector , newClassStr ,newSelector , [methodString UTF8String]);
	assert(result);
	return result;
}
